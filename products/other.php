...
if (isset($_GET['department'], $_GET['format'], $_GET['type']) && preg_match('/((\bMovies\b)|(\bTV\b))(?!;)/', $_GET['department']) /*&& preg_match('/((\b\b))(?!;)/', $_GET['format'])*/ && preg_match('/((\bgenre\b)|(\bactor\b)|(\bdirector\b)|(\bwriter\b)|(\bproducer\b)|(\bdp\b)|(\bstudio\b))(?!;)/', $_GET['type'])) {
    ...
    switch ($column_type) {
        ...
        case "genre":
            ...
            $q = "SELECT DISTINCT g.id AS genre_id, g.genre AS genres FROM genres g JOIN products_genres pg ON g.id=pg.genre_id JOIN products p ON pg.product_id=p.id JOIN departments d ON p.department_id=d.id WHERE d.department='" . $department . "' ORDER BY g.genre";
    ...
    echo $breadcrumb->build(array(ucfirst(strtolower($department_domain_name)) => strtolower($department_domain_name) . '/index.php', ucfirst($type_domain_name) => ''));
?>
    ...
    if ($r_genre) {
        ...
        while ($artist = mysqli_fetch_array($r_artist, MYSQLI_ASSOC)) {
            ...
            if (empty($artist['artist_mn'])) {
                $artists[] = '<li><a href="/FilmIndustry/eCommerce/films/index.php?type=' . $column_type . '&name=' . $artist_id_encrypt . '">' . $artist['artist_fn'] . ' ' . $artist['artist_ln'] . '</a></li>';
            } else {
                $artists[] = '<li><a href="/FilmIndustry/eCommerce/films/index.php?type=' . $column_type . '&name=' . $artist_id_encrypt . '">' . $artist['artist_fn'] . ' ' . $artist['artist_mn'] . ' ' . $artist['artist_ln'] . '</a></li>';
            }
