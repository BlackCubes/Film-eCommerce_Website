...
if (isset($_GET['department'], $_GET['format'], $_GET['type']) && preg_match('/((\bMovies\b)|(\bTV\b))(?!;)/', $_GET['department']) && preg_match('/((\bBlu-Ray\b)|(\b4K-UHD\b)|(\bDVD\b))(?!;)/', $_GET['format']) && preg_match('/((\bgenre\b)|(\bactor\b)|(\bdirector\b)|(\bwriter\b)|(\bproducer\b)|(\bdp\b)|(\bstudio\b))(?!;)/', $_GET['type'])) {
...
<?php
    ...
    if ($r_genre) {
        ...
        while ($genre = mysqli_fetch_array($r_genre, MYSQLI_ASSOC)) {
            ...
            $genres[] = '<li><a title="' . $genre['genres'] . '" href="/FilmIndustry/eCommerce/' . strtolower($department_domain_name) . '/index.php?type=' . $column_type . '&name=' . $genre_id_encrypt . '">' . $genre['genres'] . '</a></li>';
        }
