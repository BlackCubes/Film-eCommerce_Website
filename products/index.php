...
if (isset($_GET['isd']) /*&& validate_url('http://localhost/FilmIndustry/eCommerce/products/index.php?isd=' . $_GET['isd'] . '&department=' . $_GET['department'] . '&format=' . $_GET['format'])*/) {
    ...
    if (mysqli_num_rows($r) == 1) {
        ...
        while ($producer = mysqli_fetch_array($r_producer, MYSQLI_ASSOC)) {
            echo '<tr><td class="producer-image--product-view"></td class="producer-name--product-view"><td><a href="#">' . $producer['producer_fn'] . ' ' . $producer['producer_mn'] . ' ' . $producer['producer_ln'] . '</a></td></tr>';
        }
        ...
        echo '</tbody></table></div></div><div class="extra-desc-product-info--view"><div class="extra-desc-title--product-view"><h2>Extra Description</h2></div><div class="extra-desc-info--product-view"><p>' . $product['product_more_desc'] . '</p></div></div><div class="product-details-info--view-products"><div class="product-details-title--product-view"><h2>Product Details</h2></div><table class="product-details-list--product-view"><tbody><tr><td class="medium-title">Medium</td><td class="medium-type">' . $product['product_department'] . '</td></tr><tr><td class="format-title">Format</td><td class="format-type">' . $product['product_format'] . '</td></tr><tr><td class="disc-title">Number of discs</td><td class="disc-type">' . $product['product_discs'] . '</td></tr><tr><td class="rating-title">Rated</td><td class="rated-type">' . $product['product_rated'] . '</td></tr><tr><td class="theatre-date-title">Theatrical Release Date</td><td class="theatre-date">' . $product['product_release_date'] . '</td></tr><tr><td class="home-media-date-title">Home Media Date</td><td class="home-media-date">' . $product['product_media_date'] . '</td></tr><tr><td class="runtime-title">Run Time</td><td class="runtime">' . $product['product_runtime'] . '</td></tr><tr><td class="genre-title">Genre</td><td class="genre-type"></td></tr><tr><td class="sku-title">SKU</td><td class="sku-type">' . $product['product_sku'] . '</td></tr><tr><td class="isd-title">ISD</td><td class="isd-type">' . $product['product_isd'] . '</td></tr></tbody></table></div></div>';
