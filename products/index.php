...
if (isset($_GET['isd']) /*&& validate_url('http://localhost/FilmIndustry/eCommerce/products/index.php?isd=' . $_GET['isd'] . '&department=' . $_GET['department'] . '&format=' . $_GET['format'])*/) {
    ...
    if (mysqli_num_rows($r) == 1) {
        ...
        while ($actor = mysqli_fetch_array($r_actor, MYSQLI_ASSOC)) {
            echo '<td class="actor-image--product-view"></td><td class="actor-name--product-view">' . $actor['actor_fn'] . ' ' . $actor['actor_mn'] . ' ' . $actor['actor_ln'] . '</td>';
        }

        echo '</tr></tbody></table></div><div class="extra-desc-product-info--view"></div><div class="details-product-info--view"></div></div>';
