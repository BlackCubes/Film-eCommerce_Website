...
<div class="breadcrumb">' . $breadcrumb->build(array('Writers' => 'writers/index.php')) . '</div>
<div class="sidebar-a">
    <div class="results">
        <p><b>Gender</b></p>
...
$q = "SELECT DISTINCT w.id AS id, w.first_name AS first_name, w.middle_name AS middle_name, w.last_name AS last_name, w.img AS img FROM writers AS w JOIN products_writers AS pw ON w.id=pw.writer_id JOIN products AS p ON pw.product_id=p.id $where ORDER BY w.last_name LIMIT $start, $display";
$r = mysqli_query($dbc, $q) or trigger_error("Query: $q\n<br>MySQL Error " . mysqli_error($dbc));
...
while ($writer = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
    ...
    echo '<div class="container-artist"><div class="artist-image"><a href="/FilmIndustry/eCommerce/artists/profile/index.php?role=writer&artist=' . $writer_id_encrypt . '"><img src="/FilmIndustry/eCommerce/img/' . $writer['img'] . '" alt="#"></a></div><div class="artist-name">' . $writer['first_name'] . ' ' . $writer['middle_name'] . ' ' . $writer['last_name'] . '</div></div>';
