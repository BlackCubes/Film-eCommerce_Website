...
if (isset($_GET['role'], $_GET['company']) && preg_match('/(\bstudio\b)(?!;)/', $_GET['role'])) {
    ...
    if (is_numeric($id_decrypt) && preg_match('/^[1-9]{1}([0-9]{1,10})?$/', $id_decrypt)) {
        ...
        if ((mysqli_num_rows($r) == 1) && ($selected_id['id'] === $id)) {
            ...
            echo $breadcrumb->build(array(ucfirst($role_table) => $role_table . 'index.php', $studio['studio_name'] => ''));

            echo '<div class="container--studio-view"><div class="main-info--studio-view"><div class="studio-name--studio-view"><h2>' . $studio['studio_name'] . '</h2></div><div class="image--studio-view"><img alt="' . $studio['studio_name'] . '" title="' . $studio['studio_name'] . ' logo" src="/FilmIndustry/eCommerce/img/' . $studio['studio_img'] . '"></div></div><div class="product-studio-info--studio-view"><div class="product-studio-title--studio-view"><h2>Filmography</h2></div><div class="product-studio-list--studio-view">';
