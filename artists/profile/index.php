...
if (isset($_GET['artist'], $_GET['role']) && preg_match('/((\bactors\b)|(\bdirectors\b)|(\bproducers\b)|(\bdps\b)|(\bwriters\b))(?!;)/', $_GET['role'])) {
    ...
    if (is_numeric($id_decrypt) && preg_match('/^[1-9]{1}([0-9]{1,10})?$/', $id_decrypt)) {
        ...
        if ((mysqli_num_rows($r) == 1) && ($selected_id['id'] === $id)) {
            ...
            $q_pa = "SELECT p.name AS product_name, p.image_1 AS product_img, DATE_FORMAT(p.release_date, '%Y') AS product_year FROM products AS p JOIN `products_{$role}` AS pa ON p.id={$id}";
            ...
            echo '<div class="container--artist-view"><div class="main-info--artist-view"><div class="artist-name--product-view"><h2>' . $artist['artist_fn'] . ' ' . $artist['artist_mn'] . ' ' . $artist['artist_ln'] . '</h2></div><div class="image--artist-view"><img src="/FilmIndustry/eCommerce/img/' . $artist['artist_img'] . '"></div></div><div class="product-artist-info--artist-view">';
