...
if (isset($_GET['artist'], $_GET['role']) && preg_match('/((\bactor\b)|(\bdirector\b)|(\bproducer\b)|(\bdp\b)|(\bwriter\b))(?!;)/', $_GET['role'])) {
    ...
    if (is_numeric($id_decrypt) && preg_match('/^[1-9]{1}([0-9]{1,10})?$/', $id_decrypt)) {
        ...
        if ((mysqli_num_rows($r) == 1) && ($selected_id['id'] === $id)) {
            ...
            echo '<div class="container--artist-view"><div class="breadcrumb">' . $breadcrumb->build(array(ucfirst($title_name) => $link_name . '/index.php', $artist['artist_fn'] . ' ' . $artist['artist_mn'] . ' ' . $artist['artist_ln'] => '')) . '</div><div class="main-info--artist-view"><div class="artist-name--artist-view"><h2>' . $artist['artist_fn'] . ' ' . $artist['artist_mn'] . ' ' . $artist['artist_ln'] . '</h2></div><div class="image--artist-view"><img src="/FilmIndustry/eCommerce/img/' . $artist['artist_img'] . '"></div></div><div class="product-artist-info--artist-view"><div class="product-artist-title--artist-view"><h2>Filmography</h2></div><div class="product-artist-list--artist-view">';
