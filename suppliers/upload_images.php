        ...
        if ($file_error_1 === 0 && $file_error_2 === 0) {
            if ($file_size_1 < 2000000 && $file_size_2 < 2000000) {
                ...
                if (move_uploaded_file($file_tmp_name_1, $file_destination_1) && move_uploaded_file($file_tmp_name_2, $file_destination_2)) {
                    ...
                    $q = "INSERT INTO products AS p (image_1, image_2) VALUES ('{$file_new_name_1}', '{$file_new_name_2}') JOIN suppliers_products AS sp ON p.id=sp.product_id JOIN suppliers AS s ON sp.supplier_id=s.id AND s.id={$_SESSION['id']} AND p.id='{$_SESSION['product_id']}'";
