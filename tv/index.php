...
<div class="container">
    ...
    <div class="main-product">
        ...
        if (isset($_GET['p']) && is_numeric($_GET['p'])) {
            ...
            $q = "SELECT COUNT(DISTINCT p.id) FROM products p $join_table $format_join WHERE p.department_id=2 $and_where $format_where";
            $r = mysqli_query($dbc, $q) or trigger_error("Query: $q\n<br>MySQL Error: " . mysqli_error($dbc));
        ...
        $q = "SELECT DISTINCT p.name AS product_name, p.unit_price AS product_price, p.image_1 AS product_image, p.isd AS product_isd FROM products p $join_table $format_join WHERE p.department_id=2 $and_where $format_where ORDER BY p.name LIMIT $start, $display";
        $r = mysqli_query($dbc, $q) or trigger_error("Query: $q\n<br>MySQL Error " . mysqli_error($dbc));
        ...
        if ($pages > 1) {
            ...
            if ($current_page != 1) {
                echo '<a href="/FilmIndustry/eCommerce/tv/index.php?s=0&p=' . $pages . '"><i class="fas fa-caret-left"></i><i class="fas fa-caret-left"></i></a><a href="/FilmIndustry/eCommerce/tv/index.php?s=' . ($start - $display) . '&p=' . $pages . '"><i class="fas fa-caret-left"></i></a>';
            }
            ...
            if ($current_page != $pages) {
                echo '<a href="/FilmIndustry/eCommerce/tv/index.php?s=' . ($start + $display) . '&p=' . $pages . '"><i class="fas fa-caret-right"></i></a><a href="/FilmIndustry/eCommerce/tv/index.php?s=' . (($display * ($pages - 1))) . '&p=' . $pages . '"><i class="fas fa-caret-right"></i><i class="fas fa-caret-right"></i></a>';
            }
